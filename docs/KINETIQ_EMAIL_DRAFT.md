...
history, 2024-12 → 2026-09, before any deployment):
...
- 4 leg contracts implemented (KHYPELeg, SpotStakingLeg, PerpFundingLeg, BasisHedgeLeg) — ~26 KB deployed combined
- 9 interfaces in `solidity/src/interfaces/` + `IMarketDataFeed` inline in `RegimeDetector.sol`
...
1. **Sequencer revenue share** on the Yield Aggregator — a specific
   percentage of sequencer fees generated by aggregator traffic, for
   90 days of mainnet operation. We're a small team and this funds
   development through mainnet + the 4-week precompile window.
...
- Testnet deployment on the Elysium chain ID Kinetiq publishes at launch, within 2 weeks of precompile landing — **conditional on us receiving the market-data precompile address + ABI and the ElysiumCoreWriter predeploy address from Kinetiq**, both of which are still unpublished. We'll confirm the 2-week commit once those are published.
...
- Public Foundry test suite for both contracts, in the same repo, before
  mainnet.
- Open-source (MIT). We will not add any fee on top of HyperCore's
  standard trading fees or sequencer fees.
...
5. **Alpha claim caveat.** The "+3.47% APY vs static" figure is a
   measurement (25-cell sweep winner, 15 seeds, 100% positive), not a
   hypothesis. It's still thin: Liminal has $7.04M TVL and 14.50% live
   APY, so the aggregator needs to either (a) beat that on its own, or
   (b) position itself as a complementary strategy (regime-aware layer
   on top of delta-neutral), not a replacement. The email now frames it
   as (b). If you'd rather frame it as (a), you need to run more sim
   time and find a config that clears Liminal.
